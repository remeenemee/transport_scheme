...
Для работы приложения необходимо указать API ключ OpenRouteService:

1. Создайте файл `.streamlit/secrets.toml` в корне проекта
2. Добавьте в него строку `ORS_API_KEY = "ваш-ключ-здесь"`
3. При запуске приложения ключ будет загружен автоматически

> ⚠️ **Важно**: Без правильно настроенного файла `secrets.toml` с действительным API ключом, функция построения маршрутов работать не будет.
...
- Автоматически создается на основе добавленных материалов
- Показывает соответствие между цветами линий и наименованиями материалов
- Обновляется при добавлении новых поставщиков
