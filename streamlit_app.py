...
def get_save_filepath():
    """Открывает диалог выбора места для сохранения файла и возвращает путь"""
    try:
        # Инициализируем Tkinter
        root = Tk()
        root.withdraw()  # Скрываем основное окно
        root.attributes("-topmost", True)  # Поверх остальных окон
        ...
        log_error(f"Ошибка при открытии диалога сохранения: {str(e)}")
        return None

def save_map_screenshot(html_content, width=1200, height=800):
    """Сохраняет скриншот карты через временный HTML-файл"""
    try:
        # Временный файл для сохранения HTML
        temp_html = os.path.join(os.path.expanduser("~"), "temp_map.html")

        with open(temp_html, "w", encoding="utf-8") as f:
            f.write(html_content)

        # Здесь можно использовать библиотеку для создания скриншота веб-страницы
        # Примечание: это потребует установки дополнительных зависимостей
        # Например, selenium + webdriver или playwright

        # Для демонстрации просто выводим сообщение
        st.info("Карта сохранена во временный файл: " + temp_html)
        st.markdown(f"<a href='file://{temp_html}' target='_blank'>Открыть карту в браузере</a>", unsafe_allow_html=True)
        ...
        log_error(f"Ошибка при сохранении скриншота карты: {str(e)}")
        return False
...
if st.session_state.delivery_data:
    ...
    with col1:
        if st.button("📸 Сохранить карту как изображение"):
            # Получаем HTML карты
            map_html = m._repr_html_()
            save_map_screenshot(map_html)
            st.success("✅ Карта успешно сохранена!")
